...
param_dist_random = {
    ...
    'learning_rate':  [x / 100 for x in range(0, 101)],
    'max_depth': range(1,11),
    'min_samples_split': range(2,10),
    'min_samples_leaf': range(1,10),
    'subsample': [x / 100 for x in range(0, 101)],
...
random_search = RandomizedSearchCV(model, param_distributions=param_dist_random, n_iter=55, cv=3, scoring='roc_auc')
...
best_model = GradientBoostingClassifier(**best_params_combined, random_state=42)
...
df_test[["Customer Id", "Claim"]].to_csv("../Submits/submit_test_Boost_Comb.csv", index=False)