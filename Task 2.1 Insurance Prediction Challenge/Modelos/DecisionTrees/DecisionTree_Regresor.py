...
def MejorModelo(df, df_test):
    ...
    # 0.689887786
    # DecisionTreeRegressor(criterion='poisson', max_depth=4, max_features=0.8, min_samples_leaf=9, min_samples_split=3)
    # features_selected = ['Insured_Period', 'Residential','Settlement', 'Building Dimension', 'Building_Type']
    # X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=6778)
    ####################################################################
    features_selected = ['Insured_Period', 'Residential','Settlement', 'Building Dimension', 'Building_Type']
    #features_selected = df.select_dtypes(include=['int64', 'float64']).columns.tolist()[:-1]    
    X = df[features_selected]
    y = df.Claim
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=6778)
    DTC = DecisionTreeRegressor(criterion='poisson', max_depth=4, max_features=0.8, min_samples_leaf=9, min_samples_split=3)
    DTC.fit(X_train, y_train)
    df_test["Claim"] = DTC.predict(df_test[features_selected])
    df_test[["Customer Id", "Claim"]].to_csv("Submits/submit_test_DTR.csv", index = False)
...
def Randomized_Model(df):
    ...
    random_clf = RandomizedSearchCV(clf, param_distributions=param_dist, n_iter=500, cv=50, scoring='roc_auc', n_jobs=-1, verbose=1)
...
def probar_random_state(df, max_random):
    ...
    clf = DecisionTreeRegressor(criterion='poisson', max_depth=4, max_features=0.8, min_samples_leaf=9, min_samples_split=3)
